...
function cleanEgyptianPhone(phone) {
  ...
  // استخراج آخر 10 أرقام (الرقم الأساسي للموبايل المصري)
  if (digits.length >= 10) {
    const last10 = digits.slice(-10);

    // التحقق من أن الرقم يبدأ بواحد من أكواد الموبايل: 10, 11, 12, 15
    if (/^1[0125]\d{8}$/.test(last10)) {
      return last10; // نُرجع 10 أرقام فقط
    } else {
      // إذا لم يبدأ بـ 10/11/12/15، نُرجع كما هو (لكن سيكون خطأ لاحقًا بالتحقق)
      return last10;
    }
  }

  // إذا كان أقل من 10 أرقام، نُرجع كما هو (سيتم رفضه لاحقًا بالتحقق)
  return digits;
}
...
let isNavigatingToSubscription = false; // ⚠️ منع التكرار
...
function signInWithGoogle() {
  ...
  auth.signInWithPopup(provider)
    .then(userCredential => {
      ...
      // ✅ التحقق من وجود المستخدم في النظام (tenants أو subscriptions)
      Promise.all([
      ...
      .catch(error => {
        ...
        // في حالة خطأ، نُكمل كمستخدم جديد
        document.getElementById("login-section").style.display = "none";
        document.getElementById("register-form").style.display = "none";
        document.getElementById("google-details-form").style.display = "block";
        document.getElementById("google-name").value = user.displayName || user.email.split('@')[0];
...
function completeGoogleRegistration() {
  ...
  const phoneInput = document.getElementById("google-phone").value.trim();
  const errorEl = document.getElementById("google-error");
  errorEl.textContent = "";

  if (!name || !countryCode || !phoneInput) {
    errorEl.textContent = "الرجاء ملء جميع الحقول";
    return;
  }

  // تنظيف الرقم المصري
  let cleanedPhone = cleanEgyptianPhone(phoneInput);

  if (cleanedPhone.length !== 10 || !/^1[0125]\d{8}$/.test(cleanedPhone)) {
    errorEl.textContent = "أدخل رقم هاتف مصري صحيح (10 أرقام)";
    return;
  }
...
function register() {
  ...
  if (!name || !countryCode || !phoneInput || !email || !password) {
    errorEl.textContent = "املأ جميع الحقول";
    return;
  }
...
function contactSupport() {
  ...
  // استخراج الرقم النظيف (آخر 10 أرقام)
  const rawPhone = userDataToSave.phone;
...
يرجى التحقق من الدفع والرد في أسرع وقت.
  ...
  const supportPhone = "201091528121";
...
auth.onAuthStateChanged(user => {
  ...
  if (user) {
    ...
    // ✅ تحقق من وجود المستخدم في النظام
    Promise.all([
      db.ref(`tenants/${uid}`).once("value"),
      db.ref(`subscriptions/${uid}`).once("value")
    ])
